...
1. We want to add the weather information to our `appointments` tibble. We will join the two datasets adding `RH2M`, `T2M` and `PRECTOT` from `weather` to `appointments`. You want to join these by `AppointmentDay` (from `appointements`) and `YYYYMMDD` (from `weather`) - Hint: `left_join()`. Call the new dataset: `full_df`.
...
2. Check that you `full_df` dataframe has the following size:
